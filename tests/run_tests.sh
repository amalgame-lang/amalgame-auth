#!/bin/bash
# ─────────────────────────────────────────────────────────────────────
#  amalgame-auth — test runner
#  Usage: ./tests/run_tests.sh [path-to-amc]
#
#  net-http auto-attaches via a transient amalgame.lock + fake package
#  cache (it pulls async via its runtime header); datetime + crypto are
#  pure-AM facades wired with --external.
# ─────────────────────────────────────────────────────────────────────
set -u
...
for d in "$NETHTTP_DIR:facade.am" "$TLS_DIR:runtime" "$ASYNC_DIR:amalgame.toml" "$DATETIME_DIR:facade.am" "$CRYPTO_DIR:facade.am"; do
    p="${d%%:*}"; m="${d##*:}"
    [ -e "$p/$m" ] || { echo "error: dependency missing ($p/$m)"; exit 2; }
done

BUILD_DIR=$(mktemp -d -t amalgame-auth-XXXXXX)
GREEN='\033[0;32m'; RED='\033[0;31m'; NC='\033[0m'
echo "Using amc: $AMC"
cd "$PKG_DIR"

FAKE_CACHE="$BUILD_DIR/pkg_cache"
link_pkg() { local git="$2" tag="$3" sha="$4" dir="$5"; local c="$FAKE_CACHE/$git/${tag}_${sha:0:8}"; mkdir -p "$(dirname "$c")"; rm -rf "$c"; ln -s "$dir" "$c"; }
...
INC="-Iruntime -I$PKG_DIR -I$NETHTTP_DIR/runtime -I$TLS_DIR/runtime -I$ASYNC_DIR/runtime -I$DATETIME_DIR -I$CRYPTO_DIR -I$RUNTIME_DIR"

# net-http facade.am MUST be first (multi-file typedefs), then the rest.
...
"$AMC" --lib -o "$BUILD_DIR/facade" facade.am \
    ...
    --external "$CRYPTO_DIR/facade.am" 2>&1 | tail -20
...
build_and_run() {
    ...
    gcc -O2 $INC "$BUILD_DIR/$name.c" \
        "$BUILD_DIR/facade.o" "$BUILD_DIR/nethttp.o" "$BUILD_DIR/datetime.o" "$BUILD_DIR/crypto.o" \
...
build_and_run auth_test tests/auth_test.am
